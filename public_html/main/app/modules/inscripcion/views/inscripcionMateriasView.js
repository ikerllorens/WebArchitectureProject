...
App.views.InscripcionMateriasView = Backbone.View.extend({
    initialize: function () {
        console.log('Initializing Inscripcion Materias View');

        _.bindAll(this, 'populateMaterias', 'populateMateriasInfo', 'addTable', 'searchEnter');

        this.infoView = new App.views.InscripcionMateriasInfoView();
        this.collection = new App.collections.InscripcionMateriasCollection();
...
//        }
    ...
    materiaInfo: function (e) {
        ...
        $('#inscripcionMateriasInfoNombre').html(nombre);

        this.infoView.data.idMateria = id;
        var params = {
            idMateria: id,
            token: window.sessionStorage.getItem('token')
        };
    ...
    addTable: function () {
        var infoBtn = '<div class="text-left"><button class="btn btn-info materiaInfo" type="button" data-toggle="modal" data-target="#inscripcionMateriasInfoDialog"><span class="glyphicon glyphicon-info-sign"></span></button></div>';
        this.materiasTable = $('#inscripcionMateriasTable').DataTable({
            columns: [
                {data: 'clave'},
                {data: 'nombre'},
                {data: 'horas'},
    ...
    populateMateriasInfo: function () {
        $('#inscripcionMateriasInfoData').empty();
        $('#inscripcionMateriasInfoData').html(this.infoView.render().el);
    }
